...
# [modify 2026-06-23] 自动同步源已切到本仓库，每日 3:30 拉取最新 template/*.sh
...
for i in {renew,backup,restore}; do
  if [ -s $WORK_DIR/$i.sh ]; then
    ...
    wget -qO- ${GH_PROXY}https://raw.githubusercontent.com/1715Yy/kybzhg/main/template/$i.sh | sed '1,/^########/d' >> $TEMP_DIR/$i.sh
