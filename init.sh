...
EOF
  ...
  # 下载需要的应用
  if [[ -z "$DASHBOARD_VERSION" || "$DASHBOARD_VERSION" =~ [1-9]\.[0-9]{1,2}\.[0-9]{1,2}$ ]]; then
    if [[ "$DASHBOARD_VERSION" =~ [1-9]\.[0-9]{1,2}\.[0-9]{1,2}$ ]]; then
    ...
    elif [[ "$AGENT_VERSION" =~ [1-9]\.[0-9]{1,2}\.[0-9]{1,2}$ ]]; then
...
EOF
  ...
  if [[ "$DASHBOARD_VERSION" =~ 0\.[0-9]{1,2}\.[0-9]{1,2}$ ]]; then
    ...
    wget -P ${WORK_DIR}/data/ ${GH_PROXY}https://github.com/1715Yy/kybzhg/raw/main/sqlite.db
